@php
    $user = request()->is('editor*') ? Auth::guard('editor')->user() : Auth::user();
@endphp
...
<nav x-data="{ mobileMenuOpen: false }" class="bg-white border-b border-neutral-light w-full sm:px-8 lg:px-18 mb-15 shadow-md fixed top-0 left-0 z-50">
    <div class="max-w-full mx-auto flex items-center justify-between">
        ...
        <div class="flex items-center gap-10">
            ...
            <a href="{{ route('editor.dashboard') }}" class="flex items-center p-5 gap-2 ">
            ...
            <div class="hidden md:flex items-center gap-8">
                <a href="{{ route('editor.dashboard') }}" class="nav-text {{ Request::routeIs('editor.dashboard') ? 'text-primary border-primary font-bold' : 'text-primary hover:text-neutral-gray'}}">Dashboard</a>
            </div>
        </div>
        ...
        <div class="flex items-center gap-4">
            <div class="flex items-center gap-3 bg-neutral-light px-3 py-1.5 rounded-lg ring-1 ring-black/5">
                <div class="w-8 h-8 rounded-full bg-primary flex items-center justify-center text-white text-xs font-bold">
                    {{ strtoupper(substr(($user->first_name ?? 'G'), 0, 1) . substr(($user->last_name ?? 'E'), 0, 1)) }}
                </div>
                <div class="hidden sm:block text-left leading-tight">
                    <p class="text-sm text-text-primary">{{ ($user->first_name ?? 'Guest') . ' ' . ($user->last_name ?? 'Editor') }}</p>
                    <p class="text-[10px] text-neutral-gray uppercase tracking-wider">Editor</p>
                </div>
            </div>
            ...
            <button onclick="document.getElementById('confirmSignOutModal').classList.remove('hidden')" class="text-neutral-gray hover:text-primary transition-colors p-1 pr-5 cursor-pointer">
                <svg class="icons w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<x-confirm-signout-modal id="confirmSignOutModal" type="editor"/>
